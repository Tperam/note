...
![img](E:\note\语言\golang\框架源码系列\proxy&内网穿透\内网穿透.assets\各NAT IP端口映射表.png)
...
1. 端口号 40048 收到一条消息
2. 判断src.ip是否被访问过（也有可能是特殊点的情况、比如src是否符合规则1，符合则从40048；符合规则2时从40049...）
   - 如果没被访问过，则拒绝访问。
3. 获取NAT映射
...
1. 内网中随机生成一个端口，并使用该IP对外访问（192.168.1.40:38774）
2. 内网请求包到NAT层
3. 记录dst.ip+dst.port（也有可能是特殊点的情况、比如src是否符合规则1，符合则从40048；符合规则2时从40049...）
4. NAT将创建一个映射记录（1.1.1.1:40048）
5. 将五元组信息的内网ip与port改为对应的映射记录。
6. 发包
...
> https://zhuanlan.zhihu.com/p/572587706（原文：https://blog.csdn.net/yangowen/article/details/125787199）
