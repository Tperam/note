...
2. 内网A 使用go，以下代码
   ...
   func main() {
       // 定义源地址，设置自定义源端口
       srcAddr := &net.UDPAddr{
           IP:   net.IPv4zero, // IP 为 nil 代表任何 IP 可使用该端口
           Port: 12345, // 自定义源端口
       }
       ...
       go func() {
           ...
           for {
               ...
               if err != nil {
                   fmt.Printf("Error on ReadFromUDP: %v\n", err)
                   return
               }
       ...
       // 发送数据
       for  {
           fmt.Println(conn.Write([]byte(fmt.Sprintf("hello, i am LAN device 1!, currnet time:%s \n", time.Now().Format("2006-01-02 15:04:05")))))
           time.Sleep(1*time.Second)
       }
   }
   ```

   当前代码使用本机12345端口访问远程服务
...
6. 开启以下go代码
   ...
   func main() {
                   ...
                   for {
                           if len(clients) != 0 {
                                   for _, clientAddr := range clients {
   									fmt.Println(conn.WriteToUDP([]byte("ping"),clientAddr))
                                   }
                           }
           ...
           for {
                   n, cliAddr, err := conn.ReadFromUDP(buffer)
                   if err != nil {
                           fmt.Printf("Error on ReadFromUDP: %v\n", err)
                           continue
                   }
                   ...
                   clients[cliAddr.String()] = cliAddr
                   fmt.Printf("Now, %d clients connected.\n", len(clients))
   
                   for _, addr := range clients {
                           // Relay the message to other clients
                           if _, err = conn.WriteToUDP(buffer[:n], addr); err != nil {
                                   fmt.Printf("Error on WriteToUDP: %v", err)
                           }
                   }
           }
